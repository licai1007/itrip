...
<script>
	//获取到用户勾选的id，然后转换成一个数组返回
    function getSelectionsIds(){
    	var itemList = $("#itemList");//获取到table对象
    	//这是easyUI所给我们提供的方法
    ...
    var toolbar = [{
        ...
        handler:function(){
        	...
        	$("#itemEditWindow").window({
        			...
        			data.priceView = itrip.formatPrice(data.price);
        			...
        			//加载类目名称
        			$.post('/rest/item/query/itemcat/name/'+data.cid,function(_data){
        				if(_data.status == 200){
        					itrip.init({
                				"pics" : data.image,
                				"name" : _data.data.name,
                				fun:function(node){
                					itrip.changeItemParam(node, "itemeEditForm");
                				}
        					});
        				}
        			});
        ...
        iconCls:'icon-cancel',
        handler:function(){
        	var ids = getSelectionsIds();//获取到勾选项的id数组
        	if(ids.length == 0){
