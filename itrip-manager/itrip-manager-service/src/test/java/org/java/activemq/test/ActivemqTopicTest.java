...
public class ActivemqTopicTest {
	...
	public void testTopicProducerSpring() throws Exception{
		...
		Topic topic = (Topic)app.getBean("test-topic");
		jmsTemplate.send(topic,new MessageCreator() {
			@Override
			public Message createMessage(Session session) throws JMSException {
				TextMessage message = session.createTextMessage("spring send topic");
				return message;
			}
		});
	}
	@Test
	public void testTopicConsumerSpring() throws Exception{
		//读取spring的配置文件
		ApplicationContext app = new ClassPathXmlApplicationContext("classpath:spring/applicationContext-activemq.xml");
		//等待
		System.in.read();
